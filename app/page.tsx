...
export default function Home() {
  ...
  },[segments,breakdown,allStates,states,mode,startYear,startPeriod,endYear,endPeriod,output,rtoMode,pace.daily,pace.max,pace.min]);
  ...
  const checkBridge = async (codeOverride?:string) => {
    ...
    try {
      ...
      setBridgeToken("");
      setBridge("offline");
      setBridgeMessage(error instanceof Error && !error.message.includes("fetch") ? error.message : "Collector not detected. Install or start the companion, then allow local-network access if your browser asks.");
      return null;
    }
  };
  ...
  const downloadExport = async (format:"xlsx"|"csv"|"pdf", automatic=false) => {
    ...
    setDownloadNotice(`${automatic?"Collection complete. ":""}Preparing your ${format.toUpperCase()} data file…`);
  ...
  return <>
    ...
    <main id="top">
      <section className="hero"><div className="heroCopy"><p className="eyebrow"><span>01</span> PUBLIC DATA CONTROL LAYER</p><h1>Plan the scope.<br/><em>Know the load.</em><br/>Collect carefully.</h1><p className="lead">Configure public Vaahan registration intelligence by segment, state, RTO and timeline—before a single dashboard request is made.</p><div className="actions"><a className="button primary" href="#builder">Initialize collection plan <b>↗</b></a><a className="button ghost" href="#process">See the collection process</a></div><div className="heroSignals"><span><i/>EDGE + CHROME</span><span><i/>SECURE PAIRING</span><span><i/>DUPLICATE SAFE</span></div></div><aside className="telemetry"><div className="telemetryHead"><span>LIVE SAFETY ENVELOPE</span><small>VHN / CTRL / 04</small></div><div className="radar"><div className="radarCore">SAFE<small>OPERATING<br/>RANGE</small></div></div><div className="telemetryGrid"><b>1,00,000<small>MAX ROWS</small></b><b>01<small>CONCURRENCY</small></b><b>30–45<small>BALANCED / SEC</small></b><b>02<small>SAFE RETRIES</small></b></div></aside></section>
      ...
      <section id="method" className="section method"><div className="visual"><div className="visualChrome"><span>INDIA / NETWORK TOPOLOGY</span><small>AGGREGATE REGISTRATION FLOW</small></div><Image src="/vaahan-safe-collector/og.png" width={1672} height={941} sizes="(max-width: 980px) 100vw, 62vw" alt="Illustration of responsible vehicle data collection across India"/></div><div className="methodCopy"><p className="eyebrow"><span>03</span> DISTRIBUTED ARCHITECTURE</p><h2>Hosted planning.<br/><em>Minimized local collection.</em></h2><p>Free public hosting cannot safely operate a persistent browser. The planner lives online; a pairing-code and token-protected companion runs a normal Chrome or Edge window minimized on each visitor&apos;s computer, keeps data local, and pauses on access challenges.</p><ul><li><b>01</b>Chrome + Edge</li><li><b>02</b>Code + session token</li><li><b>03</b>No CAPTCHA bypass</li><li><b>04</b>Data remains local</li></ul></div></section>
...
function normalizePairingCode(value:string){return value.toUpperCase().replace(/[^A-F0-9]/g,"").slice(0,8)}
function formatPairingCode(value:string){const normalized=normalizePairingCode(value);return normalized.length>4?`${normalized.slice(0,4)}-${normalized.slice(4)}`:normalized}
