...
export default function Home() {
  ...
  const estimate = useMemo(() => {
    ...
    return {rows,rtos,reportingUnits,reports,days:Math.max(1,Math.ceil(reports/pace.daily)),hours:(reports*((pace.min+pace.max)/2)/3600).toFixed(1),errors};
  ...
  useEffect(()=>{
    if (!job || !["starting","running","waiting"].includes(job.status)) return;
  ...
  return <>
    ...
    <main id="top">
      <section className="hero"><div className="heroCopy"><p className="eyebrow"><span>01</span> PUBLIC DATA CONTROL LAYER</p><h1>Plan the scope.<br/><em>Know the load.</em><br/>Collect carefully.</h1><p className="lead">Configure public Vaahan registration intelligence by segment, state, RTO and timeline—before a single dashboard request is made.</p><div className="actions"><a className="button primary" href="#builder">Initialize collection plan <b>↗</b></a><a className="button ghost" href="#process">See the collection process</a></div><div className="heroSignals"><span><i/>EDGE + CHROME</span><span><i/>AUTO CHECKPOINT</span><span><i/>OUTPUT SAFE</span></div></div><aside className="telemetry"><div className="telemetryHead"><span>LIVE SAFETY ENVELOPE</span><small>VHN / CTRL / 02</small></div><div className="radar"><div className="radarCore">SAFE<small>OPERATING<br/>RANGE</small></div></div><div className="telemetryGrid"><b>1,00,000<small>MAX ROWS</small></b><b>01<small>CONCURRENCY</small></b><b>30–45<small>BALANCED / SEC</small></b><b>100<small>DAILY CEILING</small></b></div></aside></section>
      ...
      <section id="builder" className="section builder"><div className="sectionHead"><div><p className="eyebrow"><span>02</span> MISSION CONFIGURATOR</p><h2>Define the data envelope</h2></div><p>This free hosted control layer calculates workload and securely instructs the background Chrome or Edge companion running on your computer.</p></div><div className="sourceNotice"><b>SOURCE TRANSITION / AUG 2026</b><span>The Vaahan legacy report currently used by the collector announces retirement after 15 August 2026. The collector now detects field changes and pauses safely instead of guessing. <a href="https://analytics.parivahan.gov.in/analytics/publicdashboard/vahan?lang=en" target="_blank" rel="noreferrer">Open the announced replacement dashboard ↗</a></span></div>
        <div className="formGrid">
          ...
          <fieldset className="review"><legend><span>04</span> Review & run</legend><div className="metrics"><div><b>{format(estimate.rows)}</b><small>estimated rows</small></div><div><b>{format(estimate.reportingUnits)}</b><small>reporting units · {format(estimate.rtos)} RTOs in scope</small></div><div><b>{format(estimate.reports)}</b><small>dashboard reports</small></div><div><b>{estimate.days}</b><small>minimum days · ≈{estimate.hours}h active pacing</small></div></div>{estimate.errors.length?<div className="warning blocked">{estimate.errors.map(e=><p key={e}>{e}</p>)}</div>:<div className="warning">{rtoMode==="state_total"?"Fast state-total plan: all RTOs are combined within each selected state.":"Detailed RTO plan: each public office is collected separately."} Availability can vary on Vaahan.</div>}
            ...
            <label className="consent"><input type="checkbox" checked={consent} onChange={e=>setConsent(e.target.checked)}/><span>I understand registrations are a sales proxy and the collector must stop on CAPTCHA, rate limits or access challenges.</span></label><button className="button primary wide" disabled={!allowed||starting} onClick={()=>void startCollection()}>{starting?"Starting secure collector…":"Start minimized background collection"}</button><div className="automationNote"><b>AFTER YOU PRESS START</b><span>Each completed report is saved automatically. At the daily ceiling, the companion waits and resumes after midnight India time—no new click is needed while the computer remains awake.</span></div><a className="button secondary wide" href="/vaahan-safe-collector/vaahan-safe-companion.zip" download>Download free Windows companion</a><button className="button secondary wide" type="button" onClick={downloadPlan}>Download configuration only</button><p className="bridgeHint">First use: download and extract the companion, run <strong>install.ps1</strong> once, then keep <strong>start.ps1</strong> open. Your browser may ask for local-network access. <a href="/vaahan-safe-collector/vaahan-safe-companion.zip.sha256" target="_blank">Verify SHA-256 checksum.</a></p><a className="localLink" href={BRIDGE_URL}>Open local control room →</a>
            {job&&<div className="jobMonitor" aria-live="polite"><div className="jobTitle"><span>ACTIVE JOB / {job.id}</span><b>{job.status.toUpperCase()}</b></div><p>{job.current||"Preparing collection…"}</p><div className="progressTrack"><i style={{width:`${Math.min(100,Math.round((job.completedReports/Math.max(1,job.estimate.reportRequests))*100))}%`}}/></div><div className="jobStats"><span><b>{format(job.completedReports)}</b> / {format(job.estimate.reportRequests)} reports</span><span><b>{format(job.collectedRows)}</b> rows collected</span></div>{job.error&&<div className="jobError">{job.error}</div>}<div className="jobActions">{["starting","running","waiting"].includes(job.status)&&<button type="button" onClick={()=>void stopJob()}>STOP SAFELY</button>}{["paused","stopped"].includes(job.status)&&<button type="button" onClick={()=>void resumeJob()}>RESUME</button>}<button type="button" onClick={()=>void downloadExport(output)}>DOWNLOAD {output.toUpperCase()}</button></div></div>}
          </fieldset>
        </div>
      </section>

      <section id="process" className="section process"><div className="sectionHead"><div><p className="eyebrow"><span>03</span> COLLECTION PROCESS</p><h2>One start.<br/>Automatic checkpoints.</h2></div><p>The companion handles the routine work after launch. Keep the computer awake and the companion running; intervene only if Vaahan presents a CAPTCHA, access restriction, or an unexpected dashboard change.</p></div><div className="processRail"><article><span>01 / CONNECT</span><b>Start the companion</b><p>Run the free local companion and confirm the secure connection indicator.</p></article><article><span>02 / VALIDATE</span><b>Review the estimate</b><p>The planner blocks jobs above 1,00,000 estimated rows before any report is requested.</p></article><article><span>03 / COLLECT</span><b>Minimized browser</b><p>Chrome or Edge requests one public report at a time and saves a checkpoint after every completed report.</p></article><article><span>04 / WAIT</span><b>Daily auto-resume</b><p>At the daily ceiling, status changes to Waiting and collection restarts after midnight India time.</p></article><article><span>05 / EXPORT</span><b>Download anytime</b><p>Complete or partial results remain on this computer and can be exported from the job monitor.</p></article></div><div className="processCaution"><b>Manual action is intentionally retained for safeguards.</b><span>The tool never solves CAPTCHA, bypasses access controls, keeps a stopped PC awake, or guesses after the Vaahan page structure changes.</span></div></section>
      ...
      <section id="limits" className="section limits"><div><p className="eyebrow"><span>06</span> SAFETY PROTOCOL</p><h2>Designed to<br/>back off.</h2></div><div className="limitGrid"><article><span>01 / ACCESS</span><h3>No evasion</h3><p>No proxy rotation, fingerprint spoofing, CAPTCHA solving or login bypass.</p></article><article><span>02 / PRESSURE</span><h3>Controlled request load</h3><p>One report at a time. Balanced pacing uses 30–45 seconds and a 100-report daily ceiling.</p></article><article><span>03 / CONTINUITY</span><h3>Automatic checkpoints</h3><p>Completed reports are saved immediately; the daily ceiling triggers a wait and automatic next-day continuation.</p></article><article><span>04 / VOLUME</span><h3>Bounded output</h3><p>Plans above 1,00,000 estimated rows are blocked before collection.</p></article></div></section>
