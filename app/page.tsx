...
export default function Home() {
  ...
  return <>
    <header className="topbar"><a className="brand" href="#top"><span>V</span> Vaahan Safe Data Collector</a><nav><a href="#builder">Plan</a><a href="#method">Method</a><a href="#limits">Limits</a></nav></header>
    <main id="top">
      <section className="hero"><div><p className="eyebrow">FREE · PUBLIC AGGREGATE DATA · RESPONSIBLE COLLECTION</p><h1>Plan the scope.<br/><em>Know the load.</em><br/>Collect carefully.</h1><p className="lead">Configure public Vaahan vehicle registration data by segment, state, RTO and timeline—before placing any load on the dashboard.</p><div className="actions"><a className="button primary" href="#builder">Build a collection plan</a><a className="button ghost" href="#method">How it stays free</a></div></div><aside><span>HARD BOUNDARIES</span><b>1,00,000<small>maximum estimated rows</small></b><b>1<small>concurrent request</small></b><b>45–75s<small>randomized pause</small></b></aside></section>

      <section id="builder" className="section builder"><div className="sectionHead"><div><p className="eyebrow">FREE JOB PLANNER</p><h2>Define a responsible dataset</h2></div><p>This hosted planner is free. It creates a configuration for the visible collector running on your computer.</p></div>
      ...
      <section id="method" className="section method"><div className="visual"><img src="/vaahan-safe-collector/og.png" alt="Illustration of responsible vehicle data collection across India"/></div><div><p className="eyebrow">WHY THE HYBRID APPROACH</p><h2>Hosted planning.<br/>Visible local collection.</h2><p>Free public hosting cannot safely operate a persistent browser or let you respond when Vaahan presents an access challenge. The planner lives online; the collector stays on your computer where you can see it, stop it and resume it.</p><ul><li>No paid server</li><li>No proxy network</li><li>No CAPTCHA bypass</li><li>Data remains on your machine</li></ul></div></section>
      <section id="limits" className="section limits"><div><p className="eyebrow">SAFETY POLICY</p><h2>Designed to back off.</h2></div><div className="limitGrid"><article><span>01</span><h3>No evasion</h3><p>No proxy rotation, fingerprint spoofing, CAPTCHA solving or login bypass.</p></article><article><span>02</span><h3>Low pressure</h3><p>One report at a time with a 45–75 second randomized delay and 80-report daily limit.</p></article><article><span>03</span><h3>Bounded output</h3><p>Plans above 1,00,000 estimated rows are blocked before collection.</p></article><article><span>04</span><h3>Transparent data</h3><p>Exports retain the public source URL and collection time.</p></article></div></section>
    </main><footer><span>Vaahan Safe Data Collector</span><span>Free hosted planner · local visible collector</span></footer>
  </>;
}
